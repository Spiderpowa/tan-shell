...
contract('Shell', function(accounts) {
  ...
  it("should emit event for stdout", async function() {
    ...
    const result = await ShellContract.stdout(stream, true, {from: accounts[1]})
  ...
  it("should emit event for stderr", async function() {
    const stream = ['0x01', '0x02', '0x03', '0x04'];
    const result = await ShellContract.stderr(stream, true, {from: accounts[1]})
    assert.equal(result.logs.length, 1);
  ...
  it("should emit event for stdin", async function() {
    const stream = ['0x01', '0x02', '0x03', '0x04'];
    const result = await ShellContract.stdin(1, stream, true, {from: accounts[0]})
    assert.equal(result.logs.length, 1);
  ...
  it("should reject unregistered client", async function() {
    ...
    assert.rejects(() => {
      return ShellContract.stdout(stream, true, {from:accounts[2]});
    }, {reason: "unknown client"} );
    assert.rejects(() => {
      return ShellContract.stderr(stream, true, {from:accounts[2]});
    }, {reason: "unknown client"} );
  });
  ...
  it("should reject non owner using stdin", async function() {
    const stream = ['0x01', '0x02'];
    assert.rejects(() => {
      return ShellContract.stdin(1, stream, true, {from:accounts[1]});
  ...
  it("should prevent owner inputting unknown client index", async function() {
    const stream = ['0x01', '0x02'];
    assert.rejects(() => {
      return ShellContract.stdin(0, stream, true, {from:accounts[1]});
    }, {reason: "invalid client index"});
    assert.rejects(() => {
      return ShellContract.stdin(-1, stream, true, {from:accounts[1]});
    }, {reason: "invalid client index"});
    assert.rejects(() => {
      return ShellContract.stdin(2, stream, true, {from:accounts[1]});
    }, {reason: "unknown client"});
  });
});
